...
class DBLookupAgent:
    ...
    async def verify_order(self, order_number: str, customer_email: Optional[str] = None, session: AsyncSession = None) -> Dict[str, Any]:
        """
        Executes a secure async query to postgres to validate the order ID.
        Uses DatabaseRouter if session is not passed.
        """
        managed_session = False
        if session is None:
            session = router.get_user_history_db()
